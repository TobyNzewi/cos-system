...
_A small, durable system for running your own life with an LLM in the loop. Written for friends who already use Claude (or are about to) and want something more deliberate than a chat history. Read time: ~10 minutes. Setup time: ~60 minutes._

---

## Why I built this

For about five years I've kept a single document — a five-year personal strategy with goals, anti-goals, an annual debrief, and unfiltered self-criticism. It works. It's also lonely. The doc sits in a folder; nothing in my week pulls on it.

What I wanted was a thin, persistent layer between that long-arc strategy and my actual week. Something that:

- Knows who I am and what I'm working on, without me re-explaining every chat.
- Shows up on its own each Sunday and runs a planning session against last week's goals.
- Has continuous memory across conversations, not just within one.
- Holds at least one long-arc track — a thread of work that takes years, not days, and would otherwise get backburnered.

Off-the-shelf productivity tools don't do this. They track tasks; they don't think with you. The thing I needed was an LLM with persistent context, a calendar, and a clock. That's all this is.

This doc and the companion `cos-starter-kit` skill help you build the same shape for yourself. Your contents will be different. The structure is portable.

---

## What it is, in one paragraph

CoS is a project folder on your computer. It contains: a few files about you (a strategy doc, a résumé, anything else), one short "system context" file that tells future Claude sessions what's already been built and how to behave, and one or two scheduled tasks that run on a clock and DM you when they fire. The scheduled tasks read the project's context every time they run, so they share a memory you maintain in writing. That's the whole system. Everything else is variations on those parts.
...
**Substance** is what you're actually trying to do with your life — the long-arc tracks. Yours might be a writing project, a fitness arc, a spiritual practice, a relationship you're rebuilding, a career inflection. The scaffolding exists to serve the substance. If your operational stuff is humming and the substance work isn't moving, the system is failing at its actual job. Build the operational layer, but design every part of it to push you toward the substance.
...
├── _<track-name>.md                    ← one long-arc track (optional but recommended)
...
2. **A long-arc track doc** (`_<track-name>.md`) — captures the substance work so it survives across sessions. Not required, but the system feels hollow without it.
...
3. **Write `_system-context.md`** using the template in the starter kit. It's short — under a page. Cover: what this project is, who you are in two paragraphs, what tracks are active, what guardrails apply (preachy? no. sycophantic? no.).
4. **Pick one long-arc track.** Something you've been deferring for years. Write `_<track>.md` using the track template — it captures the goal, the framework, where you left off, and engagement notes. Don't pick three; pick one.
...
For each long-arc track: name, doc filename, one-sentence summary, why it
matters, where it lives operationally.
...
How sessions on this track run. Phases, exercises, what each session
produces.
...
The shape of useful conversation on this track. Specific.
```
...
2. ~/Documents/Claude/Projects/<your-cos>/_<track>.md  (if you have one)
...
3. Track check — explicitly ask whether the long-arc track moved this week.
...
**Preserve continuity in writing, not in chat history.** Every session should leave behind an updated file (the long-arc track, the weekly summary). Chat history isn't memory; the files are.

**One long-arc track at a time.** Three becomes none.
...
**The weekly task drifts into vagueness.** This happens when `_system-context.md` gets stale. Re-read it every few weeks; update the "active tracks" and "where I left off" sections. The doc is a living thing.

**The long-arc track stalls.** Most common failure. Usually means you picked the wrong track, or you set it up too elaborately. Simplify. The minimum viable track is one paragraph of "what I'm working on" updated weekly.
...
- `templates/_long-arc-track.md` — fill-in-the-blank track doc.
